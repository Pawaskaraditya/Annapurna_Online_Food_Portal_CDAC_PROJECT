...
import com.anapurna.dtos.ListOfFoodItemIds;
...
import com.anapurna.services.OrderItemService;
...
public class CustomerController {
	...
	public ResponseEntity<AnapurnaResponse> updateAddress(@PathVariable("id") int id,@RequestBody CustomerDto customerDto){
		boolean status=customerService.updateAddressByCustomerId(id, customerDto.getAddressText(), customerDto.getPinCode());
		if(!status) {
			return AnapurnaResponse.error("Couldn't update address");
		}
		
		return AnapurnaResponse.success("Ok");
	}
	
	
	@PostMapping("/orders/place")
	public ResponseEntity<AnapurnaResponse> placeOrder(@RequestBody PlaceOrderDto placeOrderDto){
		System.out.println(placeOrderDto);
		OrdersDto ordersDto=ordersService.addOrder(placeOrderDto);
		
		if(ordersDto==null) {
			return AnapurnaResponse.error("Couldn't add order");
			
		}
		
		return AnapurnaResponse.success(ordersDto);
	}
	
	
	@GetMapping("/orders/customer/{id}")
	public ResponseEntity<AnapurnaResponse> getAllOrdersByCustomerId(@PathVariable("id") int customerId){
		...
		if(ordersDtoList==null || ordersDtoList.isEmpty()) {
			return AnapurnaResponse.error("List is empty!");
